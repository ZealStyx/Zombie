 ...
 * drawLabels(SpriteBatch) call made by WorldItemRenderSystem after sprite drawing.
 */
public class ItemPickupSystem extends IteratingSystem {
    ...
    // For drawing item name labels — set each frame by process(), read by drawLabels()
    ...
    protected void process(int entityId) {
        ...
        int                leftover = inv.inventory.add(wi.itemId, wi.quantity);

        if (leftover == 0) {
            world.delete(bestId);
            Gdx.app.log("Pickup", "Picked up " + wi.itemId + " ×" + wi.quantity);
        } else if (leftover < wi.quantity) {
